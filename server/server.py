...
def setup_directories():
    ...
    if not os.path.exists(SAVE_DIR):
        ...
        print(f"Diretorio '{SAVE_DIR}' criado.")
...
def handle_client_connection(conn, addr):
    ...
    try:
        # recebe os 4 bytes iniciais que indicam o tamanho da imagem'
        ...
        while len(img_data) < img_size:  # repete ate chegar no tamanho esperado
            ...
            if not chunk:
                ...
                raise ConnectionError(
                    "Conexao perdida durante o recebimento da imagem.")
        ...
        # junta o caminho base com o nome da pasta do dia
        ...
        # cria o nome do arquivo da foto usando a hora exata (hora, minutos, segundo)
        filename = now.strftime("%H%M%S") + ".jpg"
        # monta o caminho completo ate a foto (nome da pasta, pasta data, nome arquivo hora)
...
def start_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # printa que servidor esta ativo
        print(f"[SERVIDOR] Ouvindo em {HOST}:{PORT}...")

        while True:
            # s.accept() bloqueia até que um cliente se conecte.
            conn, addr = s.accept()
            ...
            # inica a thread em pararelo, permitindo que o servidor volte imediamente a aceitar novas conexoes
            client_thread.start()
...
class ImageDisplayApp:
    ...
    def __init__(self, root):
        ...
        self.root = root
        # titulo da barra de ttulo da janela
        ...
        self.label = tk.Label(
            root, text="Aguardando foto...", font=("Helvetica", 16))
        # centralizado e expande para ocupar o espaço disponivel
    ...
    def check_queue(self):
        ...
        try:
            ...
            # se conseguiu, atualiza a imagem exibida no label
    ...
    def update_image(self, filepath):
        ...
        try:
            ...
            self.photo_image = ImageTk.PhotoImage(
                img)  # converte para photoimage

            # Remove o texto "Aguardando"
            self.label.config(image=self.photo_image, text="")
            self.label.image = self.photo_image  # Mantém referência
            # Atualiza o título da janela com o nome do arquivo da foto(apenas o nome do arquivo).
